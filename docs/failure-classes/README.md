...
In one atomic pass it proves a row (a) is a JSON object, (b) conforms to the closed schema - `engine` in the supported set, non-empty string `pattern` and `cue_ref` - and (c) whose `pattern` actually COMPILES under its engine (an ERE is probed with `grep -E`).
