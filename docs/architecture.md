...
A check script that prints while exiting non-zero is broken rather than actionable, and its output is never a wake: the watcher reports it once and then rate-limits it (`FM_CHECK_ERROR_COOLDOWN`), because the watcher exits on every wake and releases its lock with it, so a check that failed loudly on every sweep took supervision down on every sweep (`bin/fm-watch.sh`'s `run_check`).
...
A third, independent alarm in the same guard reads `state/.triage-duty-last.json` - the volatile result cache `bin/fm-triage-duty.sh` writes on every fleet-triage pass - and warns before a return to silent supervision if that last pass found actionable items with no owner, or if the pass itself failed to enumerate; it is a deterministic file read, never a re-enumeration, and fires independent of in-flight task count.
...
The full digest is written to `state/.subsuper-escalation-latest.md` and the injected line is only a short, hard-length-capped pointer to it (`FM_INJECT_MAX_CHARS`, default 400), so a burst of near-simultaneous captain-relevant finishes can never type a multi-KB body into the composer or strand one there on a swallowed Enter.
