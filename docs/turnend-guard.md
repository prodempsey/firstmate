...
The adapter's test seams (`FM_CODEX_SYSTEMD_FAKE_DIR`, `FM_CODEX_SYSTEMD_SYSTEMCTL`, `FM_CODEX_SYSTEMD_UNIT_DIR`) fail closed outside `FM_SUPERVISION_TEST_MODE=1` with test-owned directories, so no ambient environment variable can substitute a file for the real `systemctl --user` query in production.
`FM_SUPERVISION_TEST_MODE=1` itself is fail-closed at the shared supervision-library boundary: outside a provably test-owned home and state it errors the whole identity resolution instead of minting a synthetic identity, and the managed checkpoint service scrubs every inherited non-unit `FM_*` variable at startup, so neither ambient test mode nor inherited user-manager environment can stand in for a live verified primary (docs/codex-systemd-scheduler.md "Service runtime environment").
