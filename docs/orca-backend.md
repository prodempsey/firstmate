...
You do not need to open the app for routine supervision: from an active firstmate session, `FM_HOME=<this-firstmate-home> bin/fm-peek.sh <id>` reads a task's terminal without opening Orca, and `FM_HOME=<this-firstmate-home> bin/fm-send.sh <id> "<text>"` steers it, both unless `FM_HOME` is already set to the active firstmate home (the stable `fm-<id>` alias also works; peek is fail-closed on `FM_HOME` like send, except a fully-qualified target needs none; Enter and Ctrl-C are supported; Escape is not).
