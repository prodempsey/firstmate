...
The universal toolchain is node, git, gh with GitHub auth via `gh auth login`, no-mistakes v1.31.2 or newer, gh-axi, chrome-devtools-axi, krakendesign, compatible tasks-axi per "Backlog backend" above, and quota-axi.
This section is the single owner of that universal toolchain list; backend guides' prerequisites point here and add only their backend-specific tools.
In that list, no-mistakes runs the validation pipeline, gh-axi, chrome-devtools-axi, and krakendesign cover GitHub, browser, and rich-review operations, and tasks-axi plus quota-axi back backlog mutations and quota-balanced dispatch.
...
`tasks-axi` and `quota-axi` are required bootstrap tools in every profile, the same class as `krakendesign`.
...
`krakendesign` is the cockpit-native rich-review dock that replaced the retired `lavish-axi`/KrakenView flow; it ships with the Fleet Bridge cockpit rather than a package manager, so bootstrap validates it by presence (typically `~/.local/bin/krakendesign`) and `lavish-axi` survives only as a backward-compatible alias.
