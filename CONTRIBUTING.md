...
tests/fm-session-start.test.sh            # fm-session-start.sh: ABSENT vs empty-vs-present digest files, lock-refusal read-only path skipping every mutating step, diagnostics-first section ordering, status-tail bounding, tmux/herdr endpoint liveness, Needs FirstMate inbox digest/NEXT-STEP wiring, and composition of the real fm-lock/fm-bootstrap/fm-wake-drain scripts
tests/fm-needs-firstmate-reconcile.test.sh # fm-needs-firstmate-reconcile.sh Needs FirstMate inbox enumerator: empty fleet, scout/local-only/serving/PR-ready classification, non-terminal and secondmate exclusions, --id filtering, --json schema/counts, and SUPERSEDED backlog reclassification tests
