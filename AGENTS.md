...
1. **Never write to a project.**
   ...
   Seven sanctioned write exceptions are indexed here; their procedures live where they are used: tool-driven project initialization (section 6), fleet sync via `bin/fm-fleet-sync.sh` (sections 3, 7, and 8), local-HEAD secondmate sync via `bin/fm-bootstrap.sh` and `bin/fm-spawn.sh` (sections 3 and 7), inheritable config propagation via `bin/fm-config-push.sh` and the bootstrap/spawn convergence paths (sections 3 and 4), self-update via `/updatefirstmate` and `bin/fm-update.sh` (section 12), approved `local-only` merge via `bin/fm-merge-local.sh` (section 7), and treehouse lease reclamation via `bin/fm-lease-reclaim.sh` (section 3).
   All are fast-forward operations, guarded gitignored-config propagation, guarded local merges, or - for lease reclamation - the return of a pool worktree already proven to hold no work; none ever force, stash, or discard unlanded work.
...
config/canonical-trunk.json  the canonical trunk of every governed project - the ONE declaration of what `main` is, deliberately outside the governed repo so trunk drift cannot orphan it; LOCAL, gitignored; absence or malformation is a loud ERROR, never a pass (docs/configuration.md "Canonical trunk declaration"; verified by bin/fm-trunk-check.sh, enforced at the merge gate by bin/fm-merge-local.sh)
...
state/               volatile runtime signals; gitignored
  ...
  reconciler.check.sh   generated watcher check shim installed at locked session start; its bin/fm-reconciler.sh check lane is O(small) - it reads the cached summary and, at a bounded cadence, kicks off ONE detached refresh - so it always finishes inside FM_CHECK_TIMEOUT no matter how large the inbox (bin/fm-reconciler.sh)
  .reconciler-last.json   cached reconciler closure-proposal summary written by the full pass / detached refresh and read cheaply by the check lane; the check never re-scans the inbox, so a large inbox can never blow the watcher's check budget
  .reconciler-check-last   volatile cadence marker for the reconciler check lane; touched every pass so bin/fm-reconciler.sh check wakes and spawns a refresh only every FM_RECONCILER_CHECK_INTERVAL (default 900s); never hand-edited
  .reconciler-refresh.lock   single-holder lock so overlapping detached reconciler refreshes cannot pile up; a stale lock is stolen after FM_RECONCILER_REFRESH_MAX (default 600s); never hand-edited
  .order-audit-last.json   deterministic product of bin/fm-order.sh audit: the ACCOUNTED predicate over every non-terminal captain order; refreshed by the audit and by the reconciler full pass, read as a cheap file (never re-enumerated)
  ...
  .codex-watch-checkpoint.next.json .codex-watch-checkpoint.last.json .codex-watch-checkpoint.running.json  Codex bounded-checkpoint schedule, last-outcome, and transient runner records; docs/turnend-guard.md owns the validation contract
...
2. **Bootstrap** - detect-only diagnostics (tool/version problems, GitHub auth, the worktree-tangle check, harness override, dispatch-profile validation, backlog-backend status) always run and always print.
   ...
   The five MUTATING sweeps - fleet sync, the local secondmate fast-forward sweep, the secondmate liveness sweep, X-mode artifact writes, and treehouse lease reclamation - run only when this session actually holds the lock from step 1.
   ...
   The lease-reclamation sweep frees treehouse pool slots still leased to tasks that no longer exist, reported as `LEASE_RECLAIM:` lines (`bin/fm-lease-reclaim.sh`).
   A lease is released only by an explicit `treehouse return`, so a crewmate that dies without reaching teardown holds its slot forever and even `treehouse prune` will not take it back; left alone the pool fills with ghost leases until it hits exhaustion and blocks all new dispatch.
   The sweep frees a slot only when its owning task is provably gone and its worktree provably holds no work, and it PARKS - never returns - a dead lease still holding uncommitted or unlanded work, because discarding that work is the captain's call alone (prime directive #3).
...
Use `gh-axi` for all GitHub operations, `chrome-devtools-axi` for all browser operations, and `krakendesign` when a decision or report is complex enough to deserve a rich review surface.
...
10. Load `fleet-triage` and disposition the locked session-start digest before deciding that recovery is complete.
    Recovery has no script chokepoint of its own for the fleet-triage duty - it reconciles through the session-start digest, which already fired the `session-start` trigger (section 3) - but if THIS recovery pass itself reconciled a dead endpoint (a respawn, a re-registered secondmate), that reconciliation is its own fleet-state change and owes an explicit `bin/fm-triage-duty.sh recovery` pass before recovery is considered complete.
...
The proactive `fleet-triage` duty is a main-primary responsibility and does not relax this secondmate boundary.
...
- **local-only** - no remote, no PR.
  ...
  Review the diff with `bin/fm-review-diff.sh <id>`, relay a one-paragraph summary to the captain, and on approval run `bin/fm-merge-local.sh <id>` to fast-forward the project's DECLARED canonical trunk (`config/canonical-trunk.json`), never a guessed `main`.
  That gate is closed, not advisory: it refuses a landing aimed at anything but canonical trunk, a landing while the serving lineage is already ahead of trunk, a landing that would leave trunk behind what is running, and any missing or malformed declaration - each with the exact fix named and no ref moved.
  Treat every refusal as a stop-and-investigate and never work around it by merging by hand.
  No `fm-pr-check`. Then teardown, whose safety check requires the branch already merged into local `main`, OR the work pushed to any remote (a fork counts - relevant for upstream-contribution PRs on a local-only-registered project).
...
For proactive coordination discovered outside an existing task, load `fleet-triage`; that skill owns the reversible-action and captain-gate boundary without changing these merge rules.
...
Use chat for yes/no decisions; use krakendesign when there are multiple findings or options to triage.
...
- Relay the findings to the captain: plain chat for a focused answer, krakendesign when the report has structure worth a visual (multiple findings, options, a plan).
...
`bin/fm-wake-drain.sh` fires `wake-drain` (targeted) or `heartbeat` (full) after it drains actionable wakes, `bin/fm-teardown.sh` fires `teardown` or `scout-complete` after a closeout, `bin/fm-pr-merge.sh` and `bin/fm-merge-local.sh` fire `ship-complete` once ship work lands, and `bin/fm-session-start.sh` itself fires `session-start` as part of its own digest (section 3).
When a banner appears, load `fleet-triage`, run the pass it asks for, and record every disposition with `bin/fm-fleet-triage-record.sh` before returning to silent supervision; a terminal outcome must name its lineage, and an item that was merely seen is still actionable.
The pass records no disposition regardless of what it finds: it never records an outcome, edits the backlog, or touches a bug or task itself.
It writes only `state/.triage-duty-last.json`, its own volatile result cache, and a first-sight `surface` stamp for items the ledger has never seen, so an item's age is real and a long-ignored item stops reading like a new one.
Unhandled finished crew work is not merely banner material: while the `needs_firstmate` lane is non-empty, the turn-end guard blocks the turn end, exactly as it does for a dead watcher (`docs/turnend-guard.md`).
Arming the watcher does not discharge that lane; only landing the work or tearing it down does.
Five trigger points have no script chokepoint firstmate owns and are therefore your duty to run explicitly, by name, rather than trusting conversational memory: `recovery` once recovery reconciles a dead or respawned endpoint (section 5), `backlog-mutation` after a `tasks-axi` or hand-edited backlog change (section 10), `bug-mutation` after recording or resolving a bug through the bug CLI, `blocker-freed` after a blocker completes and its dependents come free, and `afk-exit` in the AFK-exit catch-up (below).
The pass is silent for a read-only session, which does not own triage, and while `state/.afk` exists, because the away daemon owns supervision until the captain returns.
If the enumerator itself fails mid-pass (a missing `jq`, a broken snapshot), the duty script never swallows that: it prints a distinct `ENUMERATION FAILED` banner and caches `ok: false` in `state/.triage-duty-last.json`, so `bin/fm-guard.sh`'s supervision preflight (below) keeps surfacing the outage at every later checkpoint even if this banner scrolls past - a caller's `|| true` only protects the caller's own exit status, never this signal.
...
`fm-guard.sh` carries a third, independent alarm in the same bordered style: the **fleet-triage supervision preflight**.
It reads `state/.triage-duty-last.json` - the volatile result cache `bin/fm-triage-duty.sh` writes on every pass, success or failure - and warns before you go quiet again if the last pass found actionable items with no owner, or if the last pass failed to enumerate at all.
This is a deterministic file read, never a re-enumeration: `fm-guard.sh` runs on every wake, and paying the full snapshot/NF/bug/ledger cost that often would defeat the point of a cheap guard.
It fires independent of in-flight task count, unlike the watcher-liveness alarm above, because ownerless triage items (a bug, an unreconciled report, a captain-gated backlog row) are fleet-wide and do not depend on whether any task happens to be in flight right now.
Like the other two alarms, it never blocks: the guarded operation still runs, and a read-only session gets the same warning worded as "the session holding the fleet lock owns this" rather than a repair instruction it cannot carry out.

On every verified primary harness, "no turn ends blind" has a structural backstop beyond the pull-based banner: `bin/fm-turnend-guard.sh` blocks the turn end (or forces one bounded follow-up on passive harnesses) when tasks are in flight without harness-valid supervision continuity, or when finished crew work is still unhandled (a non-empty `needs_firstmate` lane), guards both the main primary and a secondmate's own primary session, and stays silent when both are healthy.
...
Use krakendesign for multi-option decisions and structured reports worth a visual; plain chat for yes/no.
Before showing the captain any rendered UI, design, or mockup, or a structured visual report, register a KrakenDesign artifact, share its `helm.html?kd=...` review URL, and poll for the captain's feedback rather than sending screenshots; load the `krakendesign` skill for the procedure.
...
Teardown, merge, session start, and the wake drain prompt that duty for you with a banner (section 8); a backlog mutation you make by hand - filing, starting, blocking, unblocking, or closing an item - does not, because `tasks-axi` is not firstmate's script to prompt from.
Run `bin/fm-triage-duty.sh backlog-mutation` yourself right after one (or `blocker-freed` specifically when you see a blocker's dependents come free), on the same terms as any other trigger: a banner appears only if that mutation left actionable state, and every disposition it names gets recorded through `bin/fm-fleet-triage-record.sh` before you go quiet.
...
For a ship or scout task that will produce a rendered UI, design, or mockup for the captain to review, add `--kd-review` (alias `--visual-review`): the scaffold appends a visual-review contract telling the crewmate to produce one self-contained KrakenDesign HTML artifact under `data/<id>/`, register it with `krakendesign`, report the review URL, and stop for review.
...
- `krakendesign` - load before producing or relaying a rendered UI, design, mockup, rich HTML report, artifact review, visual comparison, or multi-option visual decision surface for the captain.
