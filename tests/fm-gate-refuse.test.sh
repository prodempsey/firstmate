...
SH
  ...
  # The lease is authoritative for the task worktree (the pane is never polled
  # for it), so the treehouse fake replies with the caller-chosen path.
...
test_spawn_refuses_and_admits() {
  ...
  # fm-spawn requires a leased task worktree to live under a treehouse pool path
  # (bin/fm-worktree-lib.sh), so the fixture worktree sits under .treehouse/.
  wt="$TMP/.treehouse/spawn-pool/1/spawn-wt"
  mkdir -p "$TMP/.treehouse/spawn-pool/1"
