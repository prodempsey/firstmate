...
For that gate specifically, an owner, a claim, a hold, a successor, or a bare captain batch is NOT clearance: only landing the work, tearing it down, a crew status moving off its terminal verb, a genuine `resolved`/`rejected` disposition with lineage, or a board-confirmed `fm-nf-ack.sh --to-captain` hand-off clears it - `docs/turnend-guard.md` owns the discharge rule.
